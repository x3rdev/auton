...
while True:
    ...
    # --- Read triggers ---
    # Triggers rest at -1.0 and go to +1.0 when pressed → map to 0.0–1.0
    # then apply a deadzone to kill drift (raise TRIGGER_DEADZONE if needed)
    TRIGGER_DEADZONE = 0.12

    rt_raw = (joystick.get_axis(THROTTLE_AXIS) + 1) / 2
    lt_raw = (joystick.get_axis(BRAKE_AXIS)    + 1) / 2

    rt = 0.0 if rt_raw < TRIGGER_DEADZONE else rt_raw
    lt = 0.0 if lt_raw < TRIGGER_DEADZONE else lt_raw

    raw_steer = joystick.get_axis(STEER_AXIS)

    # LT = brake only — it scales RT down to 0, never drives reverse on its own
    brake_factor  = 1.0 - lt
    effective_rt  = rt * brake_factor

    if reverse_mode:
        raw_throttle = -effective_rt   # B toggles reverse, RT still = go
    else:
        raw_throttle = effective_rt    # normal forward

    # --- Apply deadzone to steering only (triggers already cleaned above) ---
    steer    = deadzone(raw_steer, t=0.07)
    throttle = max(-1.0, min(1.0, raw_throttle))
